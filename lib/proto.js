...
pivot.handle = function(req, res, next) {
  ...
  res.on("header", function() {
    ...
    (self.callbacks.serialize || function(features, req, res, next) {
  ...
  batch.push(function(done) {
    (self.callbacks.deserialize || function(req, res, done) {
      // Default to getting a cookie
      done(null, (req.cookies ? safeParse(req.cookies.pivot) : {}));
    })(req, res, done);
  });

  // Lookup the user
  batch.push(function(done) {
    (self.callbacks.findUser || function(req, res, next) {
...
pivot._assignVariant = function(name, variants, user, done) {
  ...
  self.callbacks.lookup(name, variants, function(err, featureSettings) {
    ...
    debug("got weighting for feature '"+name+"':", featureSettings);
